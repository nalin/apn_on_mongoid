...
module APN # :nodoc:
  ...
  mattr_accessor :host
  @@host = (["production","staging"].include?(::Rails.env) ? 'gateway.push.apple.com' : 'gateway.sandbox.push.apple.com')
  ...
  @@feedback = (["production","staging"].include?(::Rails.env) ? 'feedback.push.apple.com' : 'feedback.sandbox.push.apple.com')
  ...
  # Port number for the APNS feedback
  mattr_accessor :cert
  @@cert = File.join('.', 'config', (["production","staging"].include?(::Rails.env) ? 'apns_certificate_production.pem' : 'apns_certificate_development.pem')) # THIS DONT WORK! Rails.root isnt set at this point :(
